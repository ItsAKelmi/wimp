...
function keySort(a: { key: string }, b: { key: any }) {
...
class Token {
   ...
   */
  constructor(type: string, content?: string, level?: number) {
...
export function filterTriggers(str, triggers) {
  ...
  let foundTriggers = {};
  tokens
    .filter((t) => t.type == 'highlight_start')
    .forEach((t) => (foundTriggers[t.trigger.key] = t.trigger));
...
const token_ooc_start_noparenthesis = new Token(
  'ooc_start',
  '<span class="ooc">',
);
...
const token_highlight_start = new Token(
  'highlight_start',
  '<span class="highlight">',
);
const token_highlight_end = new Token('highlight_end', '</span>');

export const oocNoParenthesis = {
  start: token_ooc_start_noparenthesis,
  end: token_ooc_end_noparenthesis,
};
...
const rules = [
  ...
  textStyle(
    /(?=^|[^\w])`/m,
    /`(?=$|[^\w])/m,
    (opt) => (opt.cmd && opt.cmd.start) || token_cmd_start,
    (opt) => (opt.cmd && opt.cmd.end) || token_cmd_end,
    {
      crossToken: false,
      processInner: (t, keepContent) =>
        t.type == typeText
  ...
  textStyle(
    /\b_/m,
    /_\b/m,
    (opt) => (opt.em && opt.em.start) || token_em_start,
    (opt) => (opt.em && opt.em.end) || token_em_end,
  ),
  // Strong
  textStyle(
    /(?=^|[^\w])\*\*/m,
    /\*\*(?=$|[^\w])/m,
    (opt) => (opt.strong && opt.strong.start) || token_strong_start,
    (opt) => (opt.strong && opt.strong.end) || token_strong_end,
  ),
  // Strikethrough
  textStyle(
    /(?=^|[^\w])~~/m,
    /~~(?=$|[^\w])/m,
    (opt) =>
      (opt.strikethrough && opt.strikethrough.start) ||
      token_strikethrough_start,
    (opt) =>
      (opt.strikethrough && opt.strikethrough.end) || token_strikethrough_end,
  ),
  // OOC
  textStyle(
    /(?=^|[^\w])\(\(/m,
    /\)\)(?=$|[^\w])/m,
    (opt) => (opt.ooc && opt.ooc.start) || token_ooc_start,
    (opt) => (opt.ooc && opt.ooc.end) || token_ooc_end,
  ),
  ...
  textStyle(
    ...
    (opt, start, end) =>
      start.match[0] == '++'
        ? (opt.sup && opt.sup.start) || token_sup_start
        : (opt.sub && opt.sub.start) || token_sub_start,
    (opt, start, end) =>
      end.match[0] == '++'
        ? (opt.sup && opt.sup.end) || token_sup_end
        : (opt.sub && opt.sub.end) || token_sub_end,
  ),
  triggers,
  // Line breaks
  textReplace(/\n/m, (m, opt) => token_br),
 ...
 */
function parseTokens(tokens: Array<Token>, opt: object, keepContent?: boolean) {
  opt = opt || defaultOpt;
...
function findInTokens(tokens, re, opts) {
  let { idx, pos, crossBoundary, crossToken, level } = Object.assign(
    { idx: 0, pos: 0, crossBoundary: true, crossToken: true, level: null },
    opts,
  );
...
function matchOffset(
  m: { match: any; offset: any; idx?: any; token?: any },
...
function textStyle(
  startRegex: RegExp,
  endRegex: RegExp | Function,
  startToken: Function,
  endToken: Function,
  config?: {
    crossToken: boolean;
    processInner: (t: any, keepContent: any) => any;
  },
) {
  let { crossToken, processInner } = Object.assign(
    { crossToken: true },
    config,
  );
  let endRegexFactory =
    typeof endRegex == 'function' ? endRegex : () => endRegex;
  return function (tokens, opt, keepContent) {
    ...
    while (idx < tokens.length) {
      ...
      let end = findInTokens(tokens, endRegexFactory(start.match), {
        idx: start.idx,
        pos: offset + 1,
        level: start.token.level,
        crossBoundary: false,
        crossToken,
      });
      if (!end) {
      ...
      spliceTextTokens(
        ...
        {
          idx: start.idx,
          from: matchOffset(start, true),
          to: matchOffset(start),
        },
...
export function isBoundary(s, idx) {
  ...
  return (
    !s.slice(idx - 1, idx).match(regexIsLetter) ||
    !s.slice(idx, idx + 1).match(regexIsLetter)
  );
}
    ...
    if (token.type == typeText) {
      ...
      for (let i = triggers.length - 1; i >= 0; i--) {
        ...
        if (trigger && typeof trigger == 'object' && trigger.key) {
          ...
          if (
            txtIdx >= 0 &&
            isBoundary(str, txtIdx) &&
            isBoundary(str, txtIdx + triggerLength)
          ) {
            spliceTextToken(
              tokens,
              idx,
              txtIdx + triggerLength,
              txtIdx + triggerLength,
              token_highlight_end,
              keepContent,
            );
            spliceTextToken(
              ...
              txtIdx,
              txtIdx,
              Object.assign({ trigger }, token_highlight_start),
              keepContent,
            );
            let t = tokens[idx + 2];
...
function spliceTextToken(tokens, idx, start, end, token, keepContent) {
  ...
  tokens.splice(
    ...
    keepContent
      ? new Token(token.type, t.content.slice(start, end), token.level)
      : token,
